...
-- Create improved analytics summary function with proper data types and error handling
...
BEGIN
  ...
  WITH current_period AS (
    SELECT
      COALESCE(SUM(NULLIF(total_orders, 0))::numeric, 0) as period_orders,
      COALESCE(SUM(NULLIF(total_revenue, 0))::numeric, 0) as period_revenue,
      COALESCE(AVG(NULLIF(total_vendors, 0))::numeric, 0) as period_vendors,
      COALESCE(AVG(NULLIF(active_vendors, 0))::numeric, 0) as period_active_vendors,
      COALESCE(SUM(NULLIF(new_vendors, 0))::numeric, 0) as period_new_vendors,
      CASE 
        WHEN SUM(NULLIF(total_orders, 0)) > 0 THEN 
          (SUM(NULLIF(total_revenue, 0)) / SUM(NULLIF(total_orders, 0)))::numeric
        ELSE 0 
  ...
  previous_period AS (
    SELECT
      COALESCE(SUM(NULLIF(total_revenue, 0))::numeric, 0) as prev_revenue
    FROM analytics_daily
